...
- from example 6.1.7 (d), it was known that if $n(x) := \tan x$ then $n'(x) = (\sec x)^2$ and it was already shown on part (c) that if we choose $a = $ and let $p(x) = x^2$ then $p'(x) = 2x$. Therefore by Chain rule
$$k'(x) = (\sec x^2)^2 \cdot 2x$$